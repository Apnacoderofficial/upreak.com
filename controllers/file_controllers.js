...
const { uploadFile }  = require('./driveUpload');
  ...
  try {
    ...
    if (req.session.role == 'Master' || req.session.role == 'Manager') {
      ...
      if (req.files['resume']) {
        ...
        const response = await uploadFile(pdfFile.path);
        googleData = {
...
exports.subscribe_list = (req, res) => {

  if (req.session.role == 'Master' || req.session.role == 'SubAdmin' || req.session.role == 'Admin' || req.session.role == 'HR' || req.session.role == 'User') {
    ContactUs.findAll()
      .then(data => {
        res.render("subcribe_list", { locals: data });
      })
      .catch(err => {
        res.redirect("/error-500");
      });
...
exports.report_resume_download = (req, res) => {

  if (req.session.role == 'Master' || req.session.role == 'SubAdmin' || req.session.role == 'Admin' || req.session.role == 'HR' || req.session.role == 'User') {
    ContactUs.findAll()
      .then(data => {
        res.render("report_resume_download", { locals: data });
      })
      .catch(err => {
        res.redirect("/error-500");
      });
...
exports.subscribe = async (req, res) => {
  try {
    ...
    const existingSubscription = await ContactUs.findOne({ where: { email: email } });
...
exports.edit_resumes = async (req, res) => {

  if (req.session.role == 'Master') {
    let data = await Resumes.findOne({ where: { id : req.query.id } });
    res.render('add_resume', {
      locals: data,
    });
  } else
...
exports.save_job_application = async (req, res) => {
    ...
    try {
        ...
        return res.json({ success: true, message: "Thank you for applying! We will review your application and get back to you soon." });
    } catch (err) {
...
exports.NewJobAlert = async (job,id) => {
  try {
    ...
    // Loop through all subscribers and send the job alert
    for (const subscriber of subscribers) {
      await sendNewJobAlertMail(
        subscriber.email,
        job.heading,
        job.location,
        ...
        job.job_type,
        job.amount,
        `https://upreak.com/jobs`
      );
    }
  } catch (err) {
    console.error("Error sending job alert:", err);
  }
};
...
exports.gemini = async (req, res) => {
  if (req.session.role !== 'Master' && req.session.role !== 'Admin') {
    ...
    if (prompt) {
      try {
        const genAI = new GoogleGenerativeAI('AIzaSyD9u017mKjnPZbvUOwI762CJL2DsQ53saE');
        ...
        res.render('gemini',{ response: result.response.text() });
      } catch (error) {
        console.error('Error generating content:', error);
        res.status(500).json({ error: 'Internal Server Error' });
      }
    } else {
      res.render('gemini', { response: '' });
    }
  }
};
...
exports.resume_builder = (req, res) => {
  res.render('resume_builder');
};
exports.resume_builder_editor = (req, res) => {
  res.render('resume-builder-editor');
};

exports.resume_templates = async (req, res) => {
  try {
    const resumes = await Resumes.findAll();  // Use findAll() to get all records from the Resumes model
    res.render('resume_templates', { resumes });  // Pass the fetched data to the view
  } catch (err) {
    console.error("Error fetching resume templates:", err);
    res.status(500).send("An error occurred while fetching resume templates.");
  }
};

exports.apply_resume = async (req, res) => {
  const { resume_id, name, email, phone_number } = req.body;

  try {
    // Check if the email already exists for the given resume ID and category
    const existingApplication = await ContactUs.findOne({
      where: {
        email: email,
    ...
    // Check if the resume exists in the Resumes table
    ...
    if (existingApplication) {
      // If the email already exists, send a thank you message without inserting a new record
      return res.json({
        success: true,
        message: "Thank you for downloading!",
        resumeLink: resume.resume_link.publicUrls || '' // Ensure this is a string and valid URL
      });
    }

    // Save the new application if email does not exist
    await ContactUs.create({
      resume_id: resume_id,
      name: name,
      email: email,
    ...
    console.log(resume.resume_link.publicUrls);

    // Return the resume link along with the success message
    return res.json({
      success: true,
      message: "Thank you for downloading!",
      resumeLink: resume.resume_link.publicUrls || '' // Ensure this is a string and valid URL
    ...
    console.error("Error applying for resume:", err);
    return res.json({ success: false, message: "There was an error processing your downloading. Please try again later." });
  }
};
