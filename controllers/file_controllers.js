...
exports.jobs = async(req, res) => {
  let data = await Jobs.findAll({ where: { status: 1 },raw : true });
...
exports.filteredJobs = async (req, res) => {
  try{
    ...
    if (req.body.experience != '') {
      whereClause.experience = (req.body.experience === '0') ? { [Op.lt]: 1 } : { [Op.gte]: 1 };
    }
...
exports.accept_slot = async (req, res) => {
  ...
  if (req.session.role == 'HR') {
    try {
      ...
      await db.meetings.update(data, { where: { id: id,
        application_id : application_id,
        status : 'Not Booked' }, raw: true, 
      });
...
exports.reset_password = async(req, res) => {
  try{
    ...
    let res_data =await User.findOne({ where : { emailid : recipientEmail } });
    if(res_data != null)
...
exports.verify_otp = async(req, res) => {
  ...
  if (verifyOTP(userEnteredOTP, storedOTP)) {
    ...
    var email  = req.session.userid;
     Response.update({phone_verify:"verified"},{where:{
        emailid:email,
    }});
...
exports.verify_otp1 = (req, res) => {
  ...
  if (verifyOTP1(userEnteredOTP, storedOTP)) {
    console.log('OTP verification successful!');
      var email  =req.session.userid;
     Response.update({whatsapp_verify:"verified"},{where:{
        emailid:email,
    }});
...
exports.verify_otp2 = (req, res) => {
  ...
  if (verifyOTP2(userEnteredOTP, storedOTP, recipientEmail)) {
      var email  =req.session.userid;
     Response.update({email_verify:"verified"},{where:{
        emailid:email,
    }});
...
exports.postlogin= async(req,res) =>{
  try {
    ...
    if (user) {
          ...
          req.session.application_id = phoneNumberResponse?.application_id || ''; // Using optional chaining (?.)
          req.session.phonenumber = phoneNumberResponse?.phonenumber || ''; // Using optional chaining (?.)
...
exports.view_report= async(req, res) => {
  ...
  if(req.session.role == 'User'){
    ...
    res.render('report',{locals:report_data,
...
exports.view_job_seekers= async(req, res) => {
  ...
  Response.findAll({ where: {urole:'User' }})
  .then(async data => {
...
exports.corporatecornerlist = async (req, res) => {
  try {
    ...
    res.render('corporatecornerlist');
  } catch (err) {
...
exports.addcorporatecorner= (req, res) => {
...
//coorporate services
  exports.addcorporateservices= (req, res) => {
    
    if(req.session.role == 'Master'){
      Promise.all([
        Corporatecorner.findAll()
    ])
        .then(([corporatecorners]) => {
  ...
  exports.save_corporateservices = (upload.single('photo'),async(req, res) => {
    ...
    if(req.session.role == 'Master'){
      ...
      console.log(data);
      corporateservices.create(data,{ raw: true })
      .then(data => {
        res.redirect("viewcorporatecorner");
  ...
  exports.editcorporateservices = (req, res) => {
    const id = req.query.id;
    
    if(req.session.role == 'Master'){
      corporateservices.findByPk(id) 
      .then(data => {
        if (data) {
          res.render('add_corporateservices',{ 
            locals: data,
...
exports.list_job_application = async (req, res) => {
  try {
    if (req.session.role != 'Master' && req.session.role != 'User') {
      return res.redirect("/error-500");
    }
...
exports.edit_job = async(req, res) => {
  try {
    if(req.session.role != 'Master' || !req.query.id)
      return res.redirect("/error-500");
...
exports.save_job_application = (upload.single('resume'), async(req, res) => {
  try { 
    ...
    res.redirect(`/job_details?id=${req.body.job_id}`);
  } catch (err) {
