...
module.exports = {
  ...
  sendTwoStepEmail: async (recipientEmail, otp) => {
    try {
  ...
  sendPasswordEmail: async (recipientEmail, password) => {
    try {
      const msg = await transporter.sendMail({
        to: recipientEmail,
        from: config.smtp.auth.user,
  ...
  sendVerificationEmail: async (recipientEmail, otp) => {
    try {
      const otpHtml = `
<html>
<head>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  ...
  sendWelcomeHrMail: async (recipientEmail, username, password) => {
    try {
      const welHtml = `
    <html>
  ...
  sendVerificationSuccessEmail: async (recipientEmail) => {
    try {
      const msg = await transporter.sendMail({
        to: recipientEmail,
        from: config.smtp.auth.user,
  ...
  sendVerificationFailureEmail: async (recipientEmail) => {
    try {
      const msg = await transporter.sendMail({
        to: recipientEmail,
        from: config.smtp.auth.user,
  ...
  sendWelcomeEmail: async (recipientEmail, password) => {
    try {
      const wlcmHtml = `<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  sendSlotConfirmEmail: async (recipientEmail, username, link, start_time, end_time) => {
    try {
      const slotHtml = `<html>
  ...
  sendHrSlotConfirmEmail: async (recipientEmail, recipientName, recipientPhone, link, start_time, end_time) => {
    try {
  ...
  sendReportDownloadEmail: async (recipientEmail) => {
    try {
  ...
  sendRequestMail: async (recipientEmail, name, service) => {
    try {
      const welHtml = `
    <html>
  ...
  sendRequestReceivedMail: async (recipientEmail, email, name, phone, services) => {
    try {
      const welHtml = `
    <html>
